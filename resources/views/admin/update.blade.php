...
@section('content')
    <div class="container-sm mt-5">
        <form action="{{ route('admin.update', ['admin' => $Barang->id]) }}" method="POST">
            @csrf
            @method('put')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control @error('nama') is-invalid @enderror" type="text" name="nama"
                                id="firstName" disabled value="{{ $Barang->nama_barang }}">
                            @error('nama')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control @error('stok') is-invalid @enderror" type="number" name="stok"
                                id="lastName" min="1" value="{{ $Barang->stok }}">
                            @error('stok')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <select name="ruangan" id="position" class="form-select">
                                @foreach ($Ruangan as $position)
                                    <option value="{{ $position->id }}"
                                        {{ old('position', $Barang->kode_ruangan) == $position->id ? 'selected' : '' }}>
